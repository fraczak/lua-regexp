...
local function copyArray(a, start, End)
    local res = {}
...
local function accepts(e)
    ...
    if (e.root == 'con') then
        return foldl(true, function(x, e) return x and accepts(e) end,
                     e.children)
    end
    if (e.root == 'plus') then
        return foldl(false, function(x, e) return x or accepts(e) end,
                     e.children)
    end
    return false
end
    ...
    if (f == nil) then
        ...
        elseif (e.root == "con") then
            ...
            for i = 1, #e.children do
                ...
                ch[i] = con(l)
                if (not accepts(e.children[i])) then break end
            end
            f = plus(ch)
        elseif (e.root == "class") then
            if (a:match(e.pattern)) then
                return ACCEPT
            else
                return REJECT
            end
...
local function normalize(t)
    ...
    local i = 2
    while i <= #r do
        if toIndex(r[i - 1]) == toIndex(r[i]) then
            table.remove(r, i)
        else
            i = i + 1
        end
...
local getToken = (function()
    local token = -- Regular expression of a token such as: w$, w+, ... 
    -- (E1.E2)*E1.E3 where:
    --   E1 = [^[+()]          -- e.g. "ala"
    --   E2 = [[][^]]*[]]      -- e.g. "ala[^ala]"
    --   E3 = [+)(]?|")*"      -- e.g. "+" or ")*"
    ...
    return function(ww)
        ...
        elseif (s:match('[(]$')) then
            ...
            result = {type = "$", subExp = decode(s), rest = w}
        end
        return result
    end
end)()
...
local function parse(word)
    ...
    local function getNextToken()
        ...
        word = token.rest
        return token.type, token.subExp
    end
